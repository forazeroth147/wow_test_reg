	...
	var user_answer=/^[\S\s]{3,10}$/;
	 function regExp(element,re,info,yaoqou)
	 {
		element.blur(function()
	    {
		  ...
		  if(user_val=='')
		  {
			 $(this).eq(0).siblings('p').html('<img src="Images/warn.png"><span>'+info+'</span></p>').css('color','#F00');
			 return false;
		  }
		  else
		  {
			  ...
			  if(user_name2)
			  {
				   $(this).eq(0).siblings('p').html('<img src="Images/yes.png">');
			  }
			  else
			  {
				  $(this).val('').focus();
				  $(this).eq(0).siblings('p').html('<img src="Images/warn.png"><span>'+yaoqou+'</span>')
	 ...
	 regExp( $('.regContent .problem3').find('input').eq(1),user_answer,'答案不得少于5个或超过10个字符','请填写正确的答案');
	  ...
	  $('.regContent .pass2').find('input').blur(function()
	      {
			...
			if(pass2=='')
			{
			 $(this).eq(0).siblings('p').html('<img src="Images/warn.png"><span>请输入确认密码</span></p>').css('color','#F00');
			 return false;
			}
			else
			{
			  ...
			  else
			  {
				  ...
				  $(this).eq(0).siblings('p').html('<img src="Images/warn.png"><span>两次密码不一致</span>')
				  $(this).eq(0).siblings('p').css({color:'#f00'});
				  return false; 
			  }
			}
	    ...
	    $(element).blur(function(){
			...
			if($(this).val()!='')
			{
				...
				if(!re_num2)
				{
					...
					$(this).siblings('p').children('.red').text(info);
					return false;
				}
...
(function(){
	...
	var self_w=$('.iframe').outerWidth();
	var self_h=$('.iframe').outerHeight();
	var l=(win_w-self_w)/2+$(window).scrollLeft();
	var t=(win_h-self_h)/2+$(window).scrollTop();
  $('.iframe').css({left:l,top:t});
	
	$('.regContent .jingao').find('font').click(function()
		{
		if($('.iframe').is(":hidden"))
		{
	...
	$('.iframe .border span').click(function(){
		
		$('.iframe').css('display','none');
		});
		
	 $('.account .account_img').click(function(){
		if($('.iframe').is(":hidden"))
		{
		 $('.iframe').css('display',' block');
		}
	...
	$('.iframe').mousedown(function(ev){
	  var Left,Top;	
	  var scr_top=$(document).scrollTop();
	  var scr_left=$(document).scrollLeft();
	  var disx=ev.clientX-$('.iframe').offset().left+scr_left;
	  var disy=ev.clientY-$('.iframe').offset().top+scr_top; 
		...
		$(document).mousemove(function(ev){
			...
			if(Left>=$(window).width()-self_w)
			{
			  Left=$(window).width()-self_w;
			}
			if(Top<=0)
			{
			  Top=0;	
			}
			if(Top>=$(window).height()-self_h)
			{
				Top=$(window).height()-self_h;
			}
...
(function(){
	var now=0;
	$('.sugge .sugge_left').find('p').each(function(index) {
		//alert($('.sugge .sugge_right').children('ul').length);
	
       $(this).click(function()
	   		{
				//alert($(this).index())
              $(this).addClass('color').siblings('p').removeClass('color');
	
		      $('.sugge .sugge_right ul').eq($(this).index()).removeClass('none').siblings('ul').addClass('none');
		   }); 
    });
	})();//个人主页切换左边tab
...
$('.reg_user').find('a').eq(1).click(function(){
	clearCookie('w_id');
	clearCookie('n_val');
